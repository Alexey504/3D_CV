...
class SFM:
    ...
    def sort_views(self):
        ...
        #--- Пример ответа --- #
        #Например, вы можете отсортировать изображения по количеству общих точек с другими изображениями
        for i in range(len(self.features)):
            if self.poses[i] is not None:
                continue
            common = 0
            for j in range(len(self.features)):
                if self.poses[j] is not None:
                    common += len(self.matches[i][j])
            views.append((i, common))
        #--- Конец примера --- #
        ...
        for j in range(len(self.features)):
            if self.poses[j] is not None:
                for match in self.matches[i][j]:
                    if self.observations_lookup.get((j, match[1])) is not None:
                        landmark_id = self.observations_lookup[(j, match[1])]
                        descriptor_id = match[0]
                        matches.add((landmark_id, descriptor_id))
        #--- Конец примера --- #
